...
function connectBananaChain() {
    const cmd = spawn('bin//multichaind.exe', ['BananaChain','-daemon']);
...
function registerBananaChain() {
    const cmd = spawn('bin//multichaind.exe', ['BananaChain@178.128.228.106:2883']);
...
function consultarSaldo(){
    const cmd = spawn('bin//multichain-cli.exe', ['BananaChain', 'gettotalbalances']);
    cmd.stdout.on('data', (data) => {
        console.log(`stdout: ${data}`);
        let buffer = data.toString();
...
//let object = JSON.parse(data);
    const cmd = spawn('bin//multichain-cli.exe', ['BananaChain','sendassettoaddress',data.cuenta,'BananaCoin',data.monto]);
    cmd.stdout.on('data', (data) => {
        console.log(`stdout: ${data}`);
    });
